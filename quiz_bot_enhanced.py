...
async def handle_share_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Create shareable invitation text
    share_text = (
        f"🎲 Hey! I found an awesome quiz for you!\n\n"
        f"📚 **{escape_markdown(title)}**\n"
        f"📝 {escape_markdown(desc) if desc else 'No description'}\n"
        f"🙋‍♂️ {total_q[0]} question(s) · ⏱ Time: {time_display}/Q\n\n"
        f"🔗 https://t.me/{bot_username}?start=quiz_{quiz_id}\n\n"
        "💪 Ready to challenge yourself?"
    )
    
    # Save share intent in context
    ...
    # Show share options
    share_options = [
        [InlineKeyboardButton("📱 Copy Link", callback_data=f"copy_share_link_{quiz_id}")],
        [InlineKeyboardButton("➡️ Forward to Chat", switch_inline_query=share_text)],
        [InlineKeyboardButton("🌐 Share to Web", url=f"https://t.me/share/url?url=https://t.me/{bot_username}?start=quiz_{quiz_id}&text=Check%20this%20quiz!")],
        [InlineKeyboardButton("🔙 Back", callback_data=f"viewq_{quiz_id}")]
    ]
    
    await query.edit_message_text(
        text=(
            f"📢 **Share Quiz: {escape_markdown(title)}**\n\n"
            f"Choose how you'd like to share:\n\n"
            f"✨ {total_q[0]} questions\n"
            f"⏱️ {time_display} per question\n"
        ),
        reply_markup=InlineKeyboardMarkup(share_options),
        parse_mode="Markdown"
    )

async def handle_copy_share_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle copy link button - provides direct link to share
    """
    query = update.callback_query
    await query.answer()
    ...
    # Record share action
    if "pending_share" in context.user_data:
        sharer_id = context.user_data["pending_share"]["sharer_id"]
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO quiz_shares (quiz_id, shared_by, shared_to_chat_id, shared_at) VALUES (?, ?, ?, ?)",
            (quiz_id, sharer_id, 0, datetime.now())  # 0 for manual/copy link share
        )
        conn.commit()
        conn.close()
    ...
    await query.edit_message_text(
        text=(
            f"✅ **Share Link Copied!**\n\n"
            f"🔗 `{share_link}`\n\n"
            f"You can now paste this link anywhere to invite others to play your quiz!\n\n"
            f"📊 **Sharing Tips:**\n"
            f"• Share in group chats for multiplayer games\n"
            f"• Send to friends for private challenges\n"
            f"• Post on your status for maximum reach"
        ),
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔄 Share Again", callback_data=f"share_quiz_{quiz_id}")],
            [InlineKeyboardButton("🔙 Back", callback_data=f"viewq_{quiz_id}")]
        ]),
        parse_mode="Markdown"
    )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_share_quiz, pattern="^share_quiz_"))
    app.add_handler(CallbackQueryHandler(handle_copy_share_link, pattern="^copy_share_link_"))
